...
class UserEmbedder:
    ...
    def gen_embs_from_observations(self, observations: List[str], userID: str, bStore: bool = True) -> List:
        tooLong = 0
        ...
        embs = model.encode(observations, convert_to_tensor=True)
    ...
    def load_msgs_from_csv(self, csvPath: str, userID: str) -> List[str]:
...
if __name__ == "__main__":
    ...
    msgs = embedder.load_msgs_from_csv(csvPath=csvPath, userID=userID)
    msgs2 = embedder.load_msgs_from_csv(csvPath=csvPath, userID=user2ID)
    ...
    emb1Mean = userutil.get_user_embs_mean(embs=embs, bStore=False)
    emb2Mean = userutil2.get_user_embs_mean(embs=embs2, bStore=False)
